...
Function list:
    ...
    27. vectorised_decode_rdt_data(infil)
    RETIRED_decode_rdt_data(infil)
...
function get_sorted_file_data(rdt_files)    # 21
###############################################
...
function vectorised_decode_rdt_data(infil)    # 27
##########################
    
    # Read binary data into a UInt8 array
    println("Reading BINARY data from ", infil)
    data_array = reinterpret(UInt8, read(infil))
    
    # Function to parse a UInt8 array into a UInt16 value
    #####################################################
    parse_uint16(array, i) = (UInt16(array[i]) << 8) | UInt16(array[i + 1])
    
    # Vectorized scan for valid record indices
    println("Scanning for valid records")
    
    # Find valid records
    record_starts = findall(i -> i + 3 <= length(data_array) && 
                             data_array[i] == 0x2a && 
                             data_array[i + 1] == 0x30 && 
                             reinterpret(Int16, (UInt16(data_array[i + 2]) << 8) | UInt16(data_array[i + 3])) == 13834, 
                             1:length(data_array))

    println("Found $(length(record_starts)) valid records.")
    num_records = length(record_starts)

    # Preallocate space based on expected number of samples
    X_date = Vector{DateTime}(undef, num_records)
    X_data = Array{Float32}(undef, 2304, num_records, 3)  # Assuming fixed `n_samples`
    GPS_errors = Array{Int16}(undef, 2304, 0)   # 2304×n for GPS error flags

    
    # Decode all valid records
    for (idx, record_start) in enumerate(record_starts)
        
        # Decode metadata
        message_length = reinterpret(Int16, (UInt16(data_array[record_start + 2]) << 8) | UInt16(data_array[record_start + 3]))
        yr = data_array[record_start + 5] + 2000
        month = data_array[record_start + 7]
        day = data_array[record_start + 8]
        hour = data_array[record_start + 9]
        minute = data_array[record_start + 10]
        utc = try DateTime(yr, month, day, hour, minute) catch; continue end

        n_samples = div(message_length - 10, 6)
        if n_samples != 2304
            println("Error: Unexpected number of samples ($n_samples) at index $record_start - skipping record!")
            continue
        end

        # Extract sample data
        sample_data = reshape(data_array[record_start + 15 : record_start + 4 + message_length], 6, n_samples)

        # Vectorized decoding of Heave, North, West
        heave_values = reinterpret(Int16, UInt16.(sample_data[1, :] .* 256 .+ sample_data[2, :])) ./ 100
        north_values = reinterpret(Int16, UInt16.(sample_data[3, :] .* 256 .+ sample_data[4, :])) ./ 100
        west_values = reinterpret(Int16, UInt16.(sample_data[5, :] .* 256 .+ sample_data[6, :])) ./ 100

        # Extract GPS error (LSB of the North values)
        gps_error = Int16.(reinterpret(UInt16, UInt16.(sample_data[3, :] .* 256 .+ sample_data[4, :])) .& 0x0001)
       
        # Store results
        X_date[idx] = utc
        X_data[:, idx, :] .= hcat(heave_values, north_values, west_values)
        GPS_errors = hcat(GPS_errors, gps_error)
        
    end

    println("Decoding complete. Processed $(count(!isnothing, X_date)) valid records.")

    return(X_date, X_data, GPS_errors)
        
end    # vectorised_decode_rdt_data()


function RETIRED_decode_rdt_data(infil)
###############################
